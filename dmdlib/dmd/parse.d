...
final class Parser(AST) : Lexer
{
    ...
    {
        from!"dmd.log".log("parseModule '%s'", mod);
    ...
    {
        from!"dmd.log".log("parseModule '%s': parseDeclDefs...", mod);
        ...
        do
        {
            ...
            //from!"dmd.log".log("parseModule '%s': parseDeclDefs: token = %s", mod, token);
            switch (token.value)
            {
            case TOK.enum_:
                {
            ...
            case TOK.colon:
            case TOK.leftCurly:
                error("(a) declaration expected, not `%s`", token.toChars());
                goto Lerror;

            case TOK.rightCurly:
            case TOK.endOfFile:
                if (once)
                    error("(b) declaration expected, not `%s`", token.toChars());
                return decldefs;
            ...
            default:
                error("(c) declaration expected, not `%s`", token.toChars());
            Lerror:
                while (token.value != TOK.semicolon && token.value != TOK.endOfFile)
                    nextToken();
                nextToken();
